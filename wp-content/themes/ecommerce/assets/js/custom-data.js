var App = (function (window) {
    ...
    return {
        ...
        SearchToggle: function () {
            ...
            $("#b-search_toggle-mob").on("click", function () {
                var head_height = $("header").height();
                var window_height = $(window).height();
                var popup_height = window_height - head_height;
                $(this).find('i').toggleClass('icon-magnifier');
...
$(document).ready(function ($) {
    ...
    $(document).on('click', 'input.b-minus', function (e) {
        let val = $('input[name="b-quantity"]').val();
        if (parseInt(val) != 1) {
            val = parseInt(val) - 1;
            $('input[name="b-quantity"]').val(val);
        }
    });
    $(document).on('click', 'input.b-plus', function (e) {
        let val = $('input[name="b-quantity"]').val();
        if ($('input[name="b-quantity"]').attr('max') > val || $('input[name="b-quantity"]').attr('max') == "") {
            val = parseInt(val) + 1;
            $('input[name="b-quantity"]').val(val);
        }
    });
